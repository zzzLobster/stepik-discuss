...
- Caddy `handle_path` strips the **full matched prefix**: public web must be `handle /discuss/web/* + uri strip_prefix /discuss` (→ `/web/*`), protected is `handle_path /discuss/*` (strips `/discuss`). Swapping them 404s remark assets.
