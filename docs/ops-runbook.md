...
## 10. Push rotation + ops (plan 0002 §7.1–§7.3)

Single-key MVP + future overlap, `_OLD` reserved; `PUSH_WEBHOOK_SECRET` simultaneous-restart window; hash-compare debug; `:8082` future option noted; direct Stepik-UI removal forbidden — only `/discuss/admin/revokeClass` revokes, reconciler bounds bypass to ~1h; Cloudflare Bypass + Bot Fight Mode OFF verification; holiday semantics §7.2.

- VAPID: single active key (`VAPID_PUBLIC_KEY_OLD` empty → all sends use current; migration paths ship as no-ops). Rotation: new → current for fresh subscribes, old kept for old rows, clients migrate via unsubscribe-before-resubscribe, retire old when old-version count → 0. No dual-send. Unknown `fp8` → `push_skip_unknown_key` + skip (never fallback). Generate via `go run ./gate/cmd/genvapid`, never commit output. Build gate with `--build-arg REV=$(git rev-parse --short=12 HEAD)` so `sw.js` REV changes and old `static-` cache is deleted on activate.
- Webhook secret: 32B hex (64 hex chars, `openssl rand -hex 32`). Rotation requires simultaneous `compose up -d --force-recreate gate remark42` window (both sides read env at boot). Debug via hash-compare (`printenv … | sha256sum`), values never leave VPS, never log headers (`X-Push-Webhook-Secret`, `X-Gate-Auth` never logged).
- Revoke: only via `/discuss/admin/revokeClass` (synchronous session rewrite + push prune in same Bolt tx, never version-bump). Direct Stepik-UI removal bypasses prune and is bounded only by VerifyTTL-freshness + 24h absent cap + hourly reconciler (~1h typical); it is forbidden in ops. Reconciler shares session sweeper ticker hourly; Stepik outage never deletes (`push_reconcile_transient`).
- Edge: Cloudflare Bypass rule `(http.host eq "stepik.study67.fyi") → Bypass cache` covers `/push/*` + `/sw.js` + `/manifest.json` + `/offline.html` (`BYPASS/DYNAMIC`, never cached). Bot Fight Mode OFF for the host — `POST /push/subscribe` must not be challenged (verify in P2 with `cf-cache-status`).
- Holiday: push is best-effort; threads retain all comments. Device offline >24h without re-auth stops receiving pushes until next page visit silently re-subscribes with stored `cids` (per-class disables preserved). Revoke/logout take effect immediately.
- Health: shallow `GET /push/health` public via `AllowPushByIP`; deep `?deep=1` requires `X-Gate-Auth` Guard via direct `http://gate:8081/push/health?deep=1` with token from docker/caddy-net (bypasses Caddy, edge deep is unconditionally 404). Deep returns counts only (`subs` via `Bucket.Stats()`, `queue` len), no PII.
- Badges: iOS — check per-app Settings → Notifications → Badges toggle + HS-installed launch with granted permission; Android — no code, OS dots the icon while notifications sit in the shade, check launcher/app notification-dot toggle.
- Future: `:8082` second listener is a future option documented only (no code in MVP).
