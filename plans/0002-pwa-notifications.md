...
Date: 2026-09-22
Status: READY FOR IMPLEMENTATION (patched 2026-09-22 — Q1-Q6 resolved, D42-D46 frozen; security first, then best UX)
...
- Author: webhook `user_name` primary (plain-text sanitized as snippet), `BestSessionForUID(author_uid).FIO` fallback, else `""` (body = snippet alone). Title: `"<Class title> — новый комментарий"` single, `"<Class title>"` summary. Summary body frozen (D42): `1 новый комментарий / 2–4 новых комментария / 5+ новых комментариев` with per-sub `N = count - ownCount`, same `tag:cid-<cid>` + `renotify:true`. Payload JSON ≤2KB. No avatar bytes, no tokens. Payload `url` asserted `strings.HasPrefix(url, "/class/")`.
...
| D24 | Revision wiring | `Server.Revision` from `main.buildRevision()` injected as `REV`; `New` signature break; prod fail-fast | Dockerfile `ARG REV + ldflags -X main.revision=$REV`; fail-fast in prod if `dev/unknown` (`dev` only for `go run`). Bare `register('/sw.js')`, no `?v=`. See D41. |
...
| D26 | Coalesce UX | Immediate-first + 30s tail-batch, frozen RU summary | Single → <5s; burst 5/30s → 2 sends (total N per-sub excl. own), visible 1. Same `tag:cid-<cid>` + `renotify:true`. Summary body frozen: `1 новый комментарий / 2–4 новых комментария / 5+ новых комментариев` (Q1). |
...
| D41 | Static MIME + Revision prod | `handleStatic`: `.js → application/javascript; charset=utf-8`, `.html → text/html; charset=utf-8`; prod REV fail-fast | Test every `STATIC_ASSETS` entry `200` + correct type. `New` signature break noted. Dockerfile stamps REV; `dev/unknown` only for `go run`. |
