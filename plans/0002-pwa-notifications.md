...
| D3.4 | `.github/workflows/ci.yml` | Add `govulncheck` job (pinned 1.x version from pkg.go.dev, never `@latest`, e.g. `go install golang.org/x/vuln/cmd/govulncheck@v1.1.4 && govulncheck ./...`). | CI green. |
...
| D12 | Dependency | `webpush-go v1.4.0` + `bluemonday` + `x/net/idna` + `govulncheck` in CI | MIT; pin `go.mod`+`go.sum`. `GenerateVAPIDKeys` = `RawURLEncoding`. `Options{Subscriber mailto:, TTL, Topic, Urgency}`. `VAPID_SUBJECT=mailto:mjgavrilov@gmail.com` per §7.1. No hand-rolled VAPID. `bluemonday.StrictPolicy` for HTML strip. `idna.ToASCII` for endpoint normalization. |
