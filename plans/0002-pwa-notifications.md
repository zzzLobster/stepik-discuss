...
Date: 2026-09-22
Status: READY FOR IMPLEMENTATION (2026-09-22 Builder pass — D1-D46 frozen; REV prod=!Placeholder+D24, badge-reuse+portrait-advisory confirmed, no parental gate frozen; security first, then best UX)
...
- Single `:8081` in MVP (no `:8082` second listener). See D36.
- Push endpoint allowlist is intentional. See D34.
...
| `POST /push/resubscribe` | `AllowPushByIP(ClientIP)` before `LoadSession`, then valid `sid` + **require-present `Origin` exact match** (no `Referer` fallback — SW JSON POST always sends `Origin`) + per-sid `AllowPush` after auth; CSRF-exempt (SW has no `document.cookie`/`localStorage`; `SameSite=Lax __Host-sid` + required Origin is sufficient; cross-site fetch carries no cookie) | Body `{old_endpoint, endpoint, keys:{p256dh, auth}, device:{ua}}` (no `cids`). Server: if `old_endpoint==""` → `400` (no INSERT; SW expiry is rare and the page-load full subscribe with `cids` + CSRF is the healer). Else row must exist and belong to same `uid`, otherwise `403` (never steal/overwrite foreign row). New `endpoint` re-runs full `IsValidEndpoint` + `IsValidKeys` (D42 — `400` on fail) before `cids` check. Copy `cids`, re-validate `cids ⊆ allowed` at resubscribe time, delete old hash, upsert new with current `sid` + `key_version`. Returns `{ok:true}`. SW `fetch(...,{credentials:'include'})`. `MaxBytesReader` 64KB. Wrong method → `405`. |
| `POST /push/webhook` | internal only, single `:8081` (no `:8082` in MVP): `r.Method==POST` else `404` + `RemoteAddr`-only loopback/private peer check (`SplitHostPort` then `net.ParseIP`, loopback + private incl. docker `172.16/12`; parse fail → deny; never read `X-Forwarded-*`/`CF-*` on this route) + deny if any of `X-Forwarded-Uri/For/Proto/Host`, `Forwarded`, `Via`, `CF-Ray/CF-Connecting-IP/CF-Visitor/CF-IPCountry`, `X-Gate-Auth` present (D45; proves the request came via Caddy/edge, not docker-internal) + `X-Push-Webhook-Secret` constant-time compare + require `Content-Type` via `mime.ParseMediaType==application/json` (allow `; charset=`, D45) + fail-closed `404` if configured secret is empty; fail-closed `404` (not `401/403`); `MaxBytesReader` 64KB before parse. Never routed via Caddy (edge has explicit `404` block). Never reuse `auth.Guard` (different secret + no `X-Gate-Auth` semantics). | remark42 POST JSON `{site, page_url, comment:{id, parent_id, user_id, user_name, text_html, text_orig, created_unix, score}}`. Gate: `ExtractCID` → `cid`; drop if unknown or `site != SITE`; strip `stepik_` prefix for `author_uid`; `comment_id` allowlist `^[A-Za-z0-9_-]{1,64}$` else drop; snippet server-side; enqueue non-blocking; return `200 {ok:true}` fast (<200ms, enqueue only). Queue-full → `push_queue_drop` + `200` (never `503`, avoids remark42 retry storm). Never log body/snippet/`user_name` on any failure path — log only `remote_hash8, bytes, reason`. |
| `GET /push/health` | shallow public `{ok:true}` rate-limited via `AllowPushByIP` only (no session). Deep `?deep=1` requires `auth.Guard` (`X-Gate-Auth`); Caddy strips inbound `X-Gate-Auth` (`request_header -X-Gate-Auth`, D46) and sends NO `header_up X-Gate-Auth` (unlike `/healthz`), so via edge deep is unconditionally `404`; ops deep works via direct `http://gate:8081/push/health?deep=1` with token from docker/caddy-net (bypasses Caddy). | Deep returns `{ok:true, subs:<push_subs count via Bucket.Stats()>, queue:<len(chan)>}` — counts only, no subscriber PII. Deep without token → `404` (not `401`). Separate from `/healthz`. |
...
- `orientation: portrait-primary` is advisory; verify iPad landscape not letterboxed in P1, drop to omit if it locks.
...
- `STATIC_ASSETS` includes `/static/push.js` (omitting it would leave bell logic network-dependent offline). Every URL in `STATIC_ASSETS` must `200` with correct `Content-Type` — unit test asserts status + type per D41. All files verified present: `gate/static/style.css, push.js (new), icon-192.png, icon-512.png, icon-512-maskable.png, apple-touch-icon.png, favicon.svg` + Gate-served `/offline.html`. `badge: /static/icon-192.png` reuse accepted for MVP; verify legibility on Android in P1, add dedicated monochrome `badge-72.png` only if illegible.
...
- Send: for each job, for each sub: skip if `sub.uid == author_uid`; if `BestSession` present require `IsTeacher || cid ∈ AllowedClassIDs` else `push_skip_revoked`; if absent: push from stored array only if `max(created_at,last_ok_at) <24h` (`cid ∈ sub.cids`); stored `"all"` + absent → skip, except teacher (`uid == TEACHER_ID` from config) → push even when absent (teacher `all` exempt from 24h absent cap, but explicit logout/delete still silences; D43). Teacher `"all"` covers future classes. Subscribe-time `cids ⊆ allowed` + send-time live check + resubscribe-time re-validate (defense in depth). Exactly one send per sub with its row key via `SendNotificationWithContext(ctx10s, payload, sub, {Subscriber: VAPID_SUBJECT, TTL: 86400, Urgency: normal, Topic: class-<cid>, VapidExpiration: default})`. Unknown `key_version` → `push_skip_unknown_key` + skip. Title truncated to ≤100 runes before marshal; `len(json)>2048` → cut snippet loop until fit. `last_ok_at` is updated only when `BestSession` was present at send time; absent-path success never touches `last_ok_at` (only `fail_count` on transient) — otherwise daily comments would self-refresh the 24h window and ex-students would never expire (D42).
- Author: webhook `user_name` primary (plain-text sanitized as snippet), `BestSessionForUID(author_uid).FIO` fallback, else `""` (body = snippet alone). Title: `"<Class title> — новый комментарий"` single, `"<Class title>"` summary. Summary body frozen with plural fn (D26): `plural(n): if n%10==1 && n%100!=11 → "новый комментарий" else if n%10 in 2..4 && n%100 not in 12..14 → "новых комментария" else "новых комментариев"` (`1 новый комментарий / 2–4 новых комментария / 5+ новых комментариев`, incl. 11–14, 111 edge); per-sub `N = count - ownCount`, same `tag:cid-<cid>` + `renotify:true`. Payload JSON ≤2KB. No avatar bytes, no tokens. Payload `url` asserted `strings.HasPrefix(url, "/class/")`.
...
- Subscribe validation: `endpoint` parses URL, `scheme==https`, `host!=""`, `userinfo==""`, `len<2048` + allowlist (D34): host must equal `fcm.googleapis.com`, `updates.push.services.mozilla.com`, or `web.push.apple.com`, or have suffix `.push.apple.com`, or suffix `.notify.windows.com` (Edge); else `400`. Normalization (D44): `strings.ToLower(strings.TrimSuffix(host,"."))`, reject empty after trim, IDNA ToASCII, require `port==""||port=="443"` (deny explicit non-443); suffix match only on dot-boundary (`host==base || HasSuffix(host,"."+base)`). IP safety: if host parses via `net.ParseIP` → deny `IsLoopback/IsPrivate/IsLinkLocalUnicast/IsUnspecified`; else resolve host → deny if any resolved IP matches those predicates. `p256dh` 87–88 chars base64url decode-check with `len(decode)==65 && [0]==0x04`, `auth` 22–24 chars decode-check with `len(decode)==16`; `cids` array requires every `cid ∈ session.allowed` (or teacher) else `403` whole request; missing/null `cids` → `400` (only explicit non-empty array or `"all"` accepted); empty `cids:[]` rejected (use `unsubscribe` instead); `"all"` allowed; `device.ua` truncate 256 + strip `Cc`+`Cf`/controls, `device.name` optional 64 + strip `Cc`+`Cf`/controls, plain-text only (never render as HTML in admin). Resubscribe re-runs full `IsValidEndpoint` + `IsValidKeys` on the new `endpoint` before `cids` check (D42); worker re-resolves host before send and applies same IP-deny (`push_skip_ssrf_rebind` + delete on match) as defense-in-depth for DNS rebind.
- CSRF/rate: `ClientIP` frozen = Caddy-authoritative `CF-Connecting-IP` (Caddy strips inbound `X-Forwarded-For` via `request_header -X-Forwarded-For` + sets `CF-Connecting-IP {client_ip}` from trusted `trusted_proxies`; Gate never trusts raw `XFF` first-entry, uses rightmost/Caddy value else `RemoteAddr` peer). `AllowPushByIP(ClientIP)` (`Every(1s)`, burst 30) checked before `LoadSession` on all `/push/*` incl. shallow `/push/health`; then `subscribe|unsubscribe` = `validSameOrigin` + `VerifyHeaderCSRF` + per-sid `AllowPush` (`Every(3s)`, burst 20 — ~20/min sustained; stale `sid` entries cleaned by existing 3-min sweep in `allow()`); `vapid-key` also per-sid `AllowPush` after auth; `resubscribe` = require-present-`Origin` exact match against `GATE_ORIGIN` (not `Host` header) + `sid` + per-sid `AllowPush` only (no `Referer` fallback). `429` returns `Retry-After` seconds. `Origin`-only exempt holds only while `__Host-sid SameSite=Lax`; if ever `None`, re-add CSRF.
- Prune wiring (atomic, single `Update` each): `handleLogout` → `DeleteSessionAndPushSubs(logged-out sid)` (delete `where row.sid==sid`, not by `uid`; idempotent double-logout keeps same-`uid` other-device rows); `admin.logoutAll` → epoch bump + delete all `push_subs` in same `Update` (no per-session iteration; epoch already invalidates all); `admin.revokeUser` → `BumpVersionAndPrunePush(target uid)` (deletes rows for target `uid`); `admin.revokeClass` → `StripClassAndPrunePush(uid,cid)` per affected `uid` (shrinks arrays, deletes single-`[cid]` rows and all `"all"` rows for that `uid`, re-subscribed as new allowed on next heal) **and synchronously rewrites affected sessions' `AllowedClassIDs` in the same Bolt tx (never bump `UserVersion` for class revoke — that would log the user out and break 0001 §5.5)** so a stale `BestSession` (6h `VerifyTTL`) cannot re-authorize the revoked `cid`. Revoke only via `/discuss/admin/revokeClass` — direct Stepik-UI removal bypasses prune and leaks up to 6h present / 24h absent (documented gap, forbidden in ops). Unit test: revoke → immediate send suppressed before next `EnsureFresh`. Hourly sweep (session sweeper ticker, shared): delete `fail_count>10 && last_ok>30d` + `no live session && max(created_at,last_ok_at)>24h` (teacher `all` rows exempt from 24h sweep per D43, but explicit logout/delete still removes them). Known gap (documented, no code in MVP): logout requires network; offline logout leaves rows until next online logout/sweep/24h cap — note in TESTLOG.
...
Safe: `handle` mutually exclusive, longest-path-first among same directive; listed paths have no overlap with `@static`, `/class/*`, `/auth/*`, `/healthz`, `/discuss/*`. Webhook has no proxy (edge always 404). `/push/health` strips inbound `X-Gate-Auth` (`request_header -X-Gate-Auth`, D46 — decision Q6) and sends NO `header_up X-Gate-Auth` (unlike `/healthz`) — deep `?deep=1` via edge is unconditionally `404` even with a guessed token; ops deep works via direct `http://gate:8081/push/health?deep=1` with token from docker/caddy-net (bypasses Caddy). `CF-Connecting-IP` + rate-limit chain unchanged. Covers `/push/webhook?x` and trailing slash via `*`.
...
Cloudflare: rule `(http.host eq "stepik.study67.fyi") → Bypass cache` confirmed 2026-09-22 and already covers new paths; verify `cf-cache-status: BYPASS/DYNAMIC` on `/push/*`, `/sw.js`, `/manifest.json`, `/offline.html` in TESTLOG. Bot Fight Mode is OFF for the host (frozen 2026-09-22, Q4) — `POST /push/subscribe` must not be challenged (verify in P2 with `cf-cache-status` evidence).
...
### D0 — spike (0.5d, standalone, before Gate changes; gates D1 — Q5)
...
Deploy order (D32): VPS `.env` (`VAPID_*`, `PUSH_WEBHOOK_SECRET`, `chmod 600`) + `deploy/compose.yml` gate `VAPID_*` + `deploy/.env.example` keys first, then gate code. `config.Load` fail-fast breaks old deploys otherwise.

1. `gate/config/config.go`: VAPID_* + secret validation (64-hex enforce, `VAPID_SUBJECT` full `mailto:`) + `VapidFP8` helper (§7.1). Fail-fast; `_OLD` empty allowed.
...
3. `gate/ratelimit/ratelimit.go`: `push` map + `AllowPush(sid) (bool, time.Duration)` (`Every(3s)`, burst 20) + `AllowPushByIP(ip) (bool, time.Duration)` (`Every(1s)`, burst 30, pre-auth; `ClientIP` = Caddy `CF-Connecting-IP`, never raw XFF first-entry).
4. `gate/push/`: leaf types (§4.1 structs + `CidsOrAll.UnmarshalJSON`), `VerifyWebhook` (`r.Method==POST` else `404` + secret constant-time + empty-secret `404` + RemoteAddr-only private peer (`SplitHostPort` then `ParseIP`, deny parse-fail, loopback + private incl. docker `172.16/12`; never read `X-*`/`CF-*` on this route) + deny if any of `X-Forwarded-Uri/For/Proto/Host`, `Forwarded`, `Via`, `CF-Ray/CF-Connecting-IP/CF-Visitor/CF-IPCountry`, `X-Gate-Auth` present (D45) + require `Content-Type` via `mime.ParseMediaType==application/json` (allow `; charset=`, D45), `404`, never-log-body), validation (`IsValidEndpoint` incl. allowlist + normalization + port `""`/`443` + resolve-then-IP-check per D34/D44 / `IsValidKeys` / `IsValidCids`, `comment_id` allowlist `^[A-Za-z0-9_-]{1,64}$`), `TitleCache`.
5. `gate/cmd/genvapid/main.go`: generator (never commit output).
6. `gate/handlers/handlers.go`: `Server.Revision` field + `New` param (set from `main.revision`; precedence: ldflags-stamped `main.revision` → `debug.ReadBuildInfo vcs.revision[:12]+-dirty` → `"dev"` for `go run`; D24); `Routes` `/push/*` + `/offline.html` (wrong-method `405`, missing/null `cids` → `400`, `Content-Type` prefix-match `application/json`); manifest upgrade (§5, `application/manifest+json`); full `sw.js` (§6, incl. `startsWith("/class/")` guard + hash-preserving `notificationclick` focus/navigate); `handleOffline`; `handleStatic` MIME fix (`.js → application/javascript; charset=utf-8`, `.html → text/html; charset=utf-8`); JSON errors (§4 mapping + `X-Robots-Tag`); `push.js` wiring (+ `maxTouchPoints` iPad check, vapid-key `429` → silent skip/retry-next-load); `AllowPushByIP` before `LoadSession` on every `/push/*` + shallow health; deep health via `auth.Guard` (Caddy strips inbound token per D46).
7. `gate/main.go`: add `var revision = "dev"` + pass to `New`, fail-fast `if !cfg.Placeholder && (revision=="dev"||revision=="unknown"||revision=="") → log.Error + os.Exit(1)` (prod = real Stepik creds, no new env var; `go run` with placeholder allows `dev`); `Dockerfile` stamps `ARG REV + RUN CGO_ENABLED=0 go build -ldflags "-X main.revision=$REV" -o /gate ./gate` with `docker build --build-arg REV=$(git rev-parse --short=12 HEAD)`; no worker yet (fan-out no-op `push_noop_no_event`).
8. `gate/static/push.js` + `gate/static/offline.html` + `class.html`/`index.html` (bell + install hint + `push.js` script + `data-cid/data-uid` + `data-allowed` for all-minus-one + `apple-touch-icon` link).
9. `gate/Dockerfile` REV stamp + `deploy/compose.yml` gate `VAPID_*` + `deploy/.env.example` keys + `deploy/Caddyfile` + `deploy/caddy-verify.sh` (checks 6–9 + stub, insertion after `/auth/*` before `/discuss` blocks, strip on `/push/health`, no `header_up`) + `docker compose config`; `caddy validate`; CF `BYPASS/DYNAMIC` verify.
10. Acceptance: Wave P1 green (teacher on iPhone iOS 26.+ + Android) + Lighthouse mobile ≥90 + `/offline.html 200 text/html` + `/static/push.js 200 application/javascript` + `/push/vapid-key 401 anon` + `govulncheck ./...` clean locally + SW bundle unit test (allowlist, no `caches.add('/')`/`/class`, no `localStorage`/`document.cookie`, helper present, every `STATIC_ASSETS` URL `200` + correct type, `navigate`-on-focus).
...
2. `push/health` deep evidence (direct `http://gate:8081/push/health?deep=1` with token from docker/caddy-net; edge deep unconditionally 404 per D46); TESTLOG waves; `caddy-verify.sh` re-run.
3. Unit tests: SW allowlist + `STATIC_ASSETS` 200 + correct `Content-Type` + hash-preserving `notificationclick` (`focus()+navigate`, hash kept) + plural `11–14/111` + all-minus-one narrow + webhook validation (`http:`/bad keys/oversize/allowlist-miss/never-log-body rejected) + endpoint-SSRF (`169.254.169.254`/`gate:8081`/`http:`/uppercase/trailing-dot/non-443 rejected `400`) + `VerifyWebhook` RemoteAddr-only (XFF spoof rejected) + header-deny (`X-Forwarded-Uri`/`Forwarded`/`Via`/`CF-Ray`/`CF-Visitor`/etc. → `404`) + `GET-webhook-404` + `charset-200` + empty-secret `404` + short-secret `404` + non-JSON `Content-Type` `404` + `BestSessionForUID` ACL matrix + revoke-immediate-suppress + absent-`max(created,last_ok)`-24h-skip + absent-no-refresh + ex-student-daily-suppressed + logout-deletes-only-that-sid + user-change guard + resubscribe-`400`-on-empty-old + resubscribe-attacker-endpoint-`400` + resubscribe-403-on-mismatch + unsubscribe-ownership (own delete 200, foreign-live 403 no-delete, orphan delete 200) + unknown-fp8-skip + `comment_id` allowlist (`/`, `<`, space rejected) + `SchemaVersion` v1→v2 + `Retry-After` cap + queue-full-`200`+`push_queue_drop` + single-`Update` atomicity.
4. `.github/workflows/ci.yml`: `govulncheck` job (pinned version, e.g. `go install golang.org/x/vuln/cmd/govulncheck@v1.1.6 && govulncheck ./...` — never `@latest`).
...
File order: `config` → `sessions` → `ratelimit` → `push` → `genvapid` → `handlers` → `admin` → `main` → `gate/Dockerfile` → `static/push.js` + `templates` + `static/offline.html` → `compose.yml` VAPID + `.env.example` (D1) → `Caddyfile` + `caddy-verify.sh` → `compose.yml` NOTIFY_* (D2) → `ci.yml` → `ops-runbook.md` → `TESTLOG.md`.
...
10. Burst: 5 comments in 30s → ≤2 pushes (1 immediate + 1 summary with frozen RU plural per D26), visible 1 (same tag + renotify). Summary `N` excludes own comments per-sub.
...
- `X-Gate-Auth` overwrite — plain `header_up` overwrites (no append); consistent with `/healthz`. `/push/health` strips inbound (`request_header -X-Gate-Auth`) and has no `header_up`, so deep via edge is unconditionally `404` (D46).
...
- Direct Stepik-UI class removal (bypasses `admin.revokeClass`) → leaks up to 6h present / 24h absent; forbidden in ops, only admin revoke revokes. Teacher stolen-device `"all"` exempt lives until explicit `logoutAll`/delete — bound to 7d `max(created,last_ok)` as future prod option, not MVP (D43 frozen).
...
| D16 | Send-time ACL | Present → require allowed else suppress; absent array + `max(created,last_ok)<24h` → push from stored; absent `"all"` → skip except teacher (exempt, D43); logout deleted → nothing; revoke prunes atomically | `push_skip_revoked` on suppress. `revoke-class` deletes `"all"` rows + synchronously rewrites sessions `AllowedClassIDs` in same tx via `StripClassAndPrunePush` (never version-bump; healed on next visit). Sweep `fail>10 && ok>30d` + `no session && max(created,last_ok)>24h` (teacher `all` exempt per D43). Ex-student >24h without re-auth gets nothing. `last_ok_at` only on present-session sends (D42). See D40/D42/D43. |
...
| D18 | Resubscribe | `POST /push/resubscribe` UPDATE-by-`old_endpoint`; empty old → `400` + full endpoint re-validation | `old==""` → `400` (no INSERT; page-load full subscribe with `cids` + CSRF is the healer); foreign `old` → `403`; new `endpoint` re-runs full `IsValidEndpoint` + `IsValidKeys` (`400` on fail, D42) then copied `cids` re-validated (`403`). SW best-effort (400-tolerant); page primary healer; compat Win/Mac/iOS/Android × Chrome/Chromium/Safari/Firefox. See D37/D42. |
...
| D24 | Revision wiring | `Server.Revision` from `main.revision` (`var revision="dev"` + ldflags stamp); `New` signature break; prod fail-fast via `!Placeholder` | Precedence: ldflags `-X main.revision=$REV` (from `ARG REV`, built via `--build-arg REV=$(git rev-parse --short=12 HEAD)`) → `debug.ReadBuildInfo vcs.revision[:12]+-dirty` → `"dev"`. Prod = `!cfg.Placeholder` (real Stepik creds, no new env var); `if prod && rev in (dev,unknown,"") → fatal`. `go run` with placeholder allows `dev`. `handleSW` injects via `%q`; bare `register('/sw.js')`, no `?v=` (byte-change drives SW update → new `STATIC_CACHE`, old deleted on activate). See D41. |
| D25 | Webhook guard | New `VerifyWebhook` (`POST`-only + secret constant-time + empty-secret `404` + RemoteAddr-only private peer + extended header-deny + `mime.ParseMediaType` JSON, `404`) | `SplitHostPort` then `ParseIP`, reject parse fail, never read `X-*`/`CF-*`. Deny if `X-Forwarded-Uri/For/Proto/Host`, `Forwarded`, `Via`, `CF-Ray/CF-Connecting-IP/CF-Visitor/CF-IPCountry`, `X-Gate-Auth` present (D45). Require `mime.ParseMediaType==application/json` (allow charset). `r.Method==POST` else `404`. Never reuse `auth.Guard`; never via Caddy. See D36/D45. |
| D26 | Coalesce UX | Immediate-first + 30s tail-batch, frozen RU summary + plural fn | Single → <5s; burst 5/30s → 2 sends (total N per-sub excl. own), visible 1. Same `tag:cid-<cid>` + `renotify:true`. Summary body frozen: `1 новый комментарий / 2–4 новых комментария / 5+ новых комментариев` + fn `n%10==1&&n%100!=11→комментарий; n%10 in 2..4&&n%100 not in 12..14→комментария; else комментариев` (Q1). |
...
| D34 | Endpoint allowlist | YES restrict — authenticated SSRF closed at subscribe `400` + normalization + send-time rebind | Allow: `fcm.googleapis.com`, `updates.push.services.mozilla.com`, `web.push.apple.com`, `*.push.apple.com`, `*.notify.windows.com` (Edge). Normalize `ToLower+TrimSuffix(".")+IDNA`, port `""`/`443` only, dot-boundary suffix (D44). Deny `IsLoopback/IsPrivate/IsLinkLocalUnicast/IsUnspecified` after `ParseIP` + resolve-then-check every IP. Reject outside list `400`. Worker re-resolves before send (`push_skip_ssrf_rebind`+delete). Zero UX cost (legit browsers only use known hosts). Unit test with `169.254.169.254` / `gate:8081` / `http:` / uppercase/trailing-dot/non-443 in D3. |
...
| D36 | Webhook header-deny + single-port | KEEP single `:8081` in MVP (no `:8082`); extended deny + `POST`-only + prefix CT | Extra listener = compose/healthcheck/complexity for <20 users; header-deny + secret + edge 404 sufficient. `:8082` is future option (runbook note only). `VerifyWebhook` denies if `X-Forwarded-Uri/For/Proto/Host`, `Forwarded`, `Via`, `CF-Ray/CF-Connecting-IP/CF-Visitor/CF-IPCountry`, `X-Gate-Auth` present (D45) + `r.Method==POST` else `404` + `mime.ParseMediaType==application/json` + fail-closed `404` if secret empty. |
...
| D39 | Health localhost-only | Strip + no inject; edge deep unconditionally 404 | Caddy `handle /push/health*` with `request_header -X-Gate-Auth` + NO `header_up X-Gate-Auth` (unlike `/healthz`, D46). Shallow rate-limited via `AllowPushByIP`. Deep `?deep=1` via edge unconditionally 404 even with token; ops deep via direct `http://gate:8081/...` with token (bypasses Caddy). Deep = subs count + queue len only, no PII. |
| D40 | Absent cap | `max(created_at,last_ok_at)<24h` for stored-array push; sweep `max(...)>24h`; no self-refresh | Closes ex-student leak and fixes fresh-sub-never-fires (pure `last_ok` would suppress a fresh sub that never fired). `last_ok_at` only on present-session sends (D42). Teacher `all` exempt per D43. |
| D41 | Static MIME + Revision prod | `handleStatic`: `.js → application/javascript; charset=utf-8`, `.html → text/html; charset=utf-8`; prod REV fail-fast via `!Placeholder` | Test every `STATIC_ASSETS` entry `200` + correct type. `New` signature break noted. `gate/Dockerfile` stamps REV (`ARG REV` + ldflags); `dev/unknown` only for `go run` with placeholder creds. |
| D42 | Absent `last_ok` + resubscribe validation | No self-refresh; resubscribe full endpoint check; frozen RU summary | Absent-path success never touches `last_ok_at` (High fix). Resubscribe new `endpoint` → full `IsValidEndpoint`+`IsValidKeys` then `cids` re-check. Summary body frozen per D26 (Q1 yes). Tests: `absent-no-refresh`, `ex-student-daily-suppressed`, `resubscribe-attacker-400`. |
| D43 | Teacher `all` exempt | Teacher stored `"all"` exempt from 24h absent cap (Q2 yes) | Explicit logout/delete still silences. Sweep skips `uid==TEACHER_ID` 24h rule. Student `"all"` + absent → skip. |
...
| D45 | Webhook strictness | `POST`-only + extended deny + prefix CT | `r.Method==POST` else `404`; deny `X-Forwarded-*/Forwarded/Via/CF-*/X-Gate-Auth`; `mime.ParseMediaType==application/json`. Tests: `Forwarded-404`, `charset-200`, `GET-404`. |
| D46 | Health strip (Q6 decided) | Strip inbound `X-Gate-Auth` on `/push/health*` | `request_header -X-Gate-Auth` + no `header_up` → edge deep unconditionally `404` (strongest; token-guess via edge impossible). Ops via direct `gate:8081`. Chosen over token-guarded because security first and ops cost is one `docker exec` line. |
...
Assumptions lists only env/edge/device assumptions; D35/D37/D38/D39/D40/D41/D42/D43/D44/D45/D46 are normative in §12, not repeated here.

- Quiet-hours none in MVP (D2) — acceptable for school use; revisit on night-spam complaint.
- Teacher `"all"` auto-includes future classes (live `BestSession` expansion) + exempt from 24h absent cap (D43, Q2 yes); explicit logout/delete still silences — desired; no extra setting.
- Gate `512` vs remark42 `200` queue mismatch intentional (gate absorbs bursts + retries).
- iPad includes `maxTouchPoints>1` check alongside `Macintosh` UA (implemented, not optional).
- `VAPID_SUBJECT=mailto:mjgavrilov@gmail.com` — frozen 2026-09-22, no new contact needed.
- Cloudflare Bypass rule `(http.host eq "stepik.study67.fyi")` — confirmed 2026-09-22, covers `/push/*`, `/sw.js`, `/manifest.json`, `/offline.html`; `BYPASS`/`DYNAMIC` recorded in TESTLOG P1/P2; Bot Fight Mode OFF frozen 2026-09-22 (Q4) — `POST /push/subscribe` not challenged, verified in P2.
- iPhone iOS 26.+ + Android Chrome test devices — confirmed 2026-09-22.
- Endpoint allowlist (D34/D44) intentional — zero UX cost, closes authenticated SSRF.
- Single `:8081` for webhook (D36/D45) intentional — `:8082` is future option only.
- Offline logout leaves rows until online logout/sweep/24h cap (Q3 documented gap, no code in MVP).
- D0 gates D1 (Q5 yes): no Gate changes without `ID` charset + `Locator.URL` shape + `Orig`/`Unix` + anchor screenshot verdict.
