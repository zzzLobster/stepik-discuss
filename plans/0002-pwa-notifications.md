...
- Current-state gaps closed by §9: `Server` has no `Revision` field (`main.buildRevision()` exists but is not injected); `Routes()` has no `/push/*` or `/offline.html`; `config.Load()` is fail-fast but has no VAPID/PUSH vars; `sessions` has 4 buckets only (`sessions`, `user_versions`, `teacher_token`, `meta`), no `SessionsForUID`/`BestSessionForUID`; `ratelimit` has no `AllowPush`/`AllowPushByIP`; `static/push.js` + `static/offline.html` missing; `webpush-go` absent from `go.mod`; CI has no `govulncheck`; Caddyfile has no `/push/*` or `/offline.html`; compose has no `NOTIFY_*`/`VAPID_*`; `caddy-verify.sh` checks 1–5 only. `icon-512-maskable.png` file exists but is unused. `handleStatic` lacks `.js → application/javascript` and `.html → text/html` mappings. Bolt has no `push_subs`/`push_meta` buckets and no `SchemaVersion` bump for them.
...
| `POST /push/resubscribe` | `AllowPushByIP(ClientIP)` before `LoadSession`, then valid `sid` + **require-present `Origin` exact match against `cfg.Origin`** (no `Referer` fallback — SW JSON POST always sends `Origin`) + per-sid `AllowPush` after auth; CSRF-exempt (SW has no `document.cookie`/`localStorage`; `SameSite=Lax __Host-sid` + required Origin is sufficient; cross-site fetch carries no cookie) | Body `{old_endpoint, endpoint, keys:{p256dh, auth}, device:{ua}}` (no `cids`). Server: if `old_endpoint==""` → `400` (no INSERT; SW expiry is rare and the page-load full subscribe with `cids` + CSRF is the healer). Else row must exist and belong to same `uid`, otherwise `403` (never steal/overwrite foreign row). New `endpoint` re-runs full `IsValidEndpoint` + `IsValidKeys` (`400` on fail) before `cids` check. Copy `cids`, re-validate `cids ⊆ allowed` at resubscribe time, delete old hash, upsert new with current `sid` + `key_version`. Returns `{ok:true}`. SW `fetch(...,{credentials:'include'})`. `MaxBytesReader` 64KB. Wrong method → `405`. |
| `POST /push/webhook` | internal only, single `:8081`: `r.Method==POST` else `404` + `RemoteAddr`-only loopback/private peer check (`SplitHostPort` then `net.ParseIP`, loopback + private incl. docker `172.16/12`; parse fail → deny; never read `X-Forwarded-*`/`CF-*` on this route) + deny if any of `X-Forwarded-Uri/For/Proto/Host`, `Forwarded`, `Via`, `CF-Ray/CF-Connecting-IP/CF-Visitor/CF-IPCountry`, `X-Gate-Auth` present (proves the request came via Caddy/edge, not docker-internal) + `X-Push-Webhook-Secret` constant-time compare + require `Content-Type` via `mime.ParseMediaType==application/json` (allow `; charset=`) + fail-closed `404` if configured secret is empty; fail-closed `404` (not `401/403`); `MaxBytesReader` 64KB before parse. Never routed via Caddy (edge has explicit `404` block). Never reuse `auth.Guard` (different secret + no `X-Gate-Auth` semantics). | remark42 POST JSON `{site, page_url, comment:{id, parent_id, user_id, user_name, text_html, text_orig, created_unix, score}}`. Gate: `ExtractCID` → `cid`; drop if unknown or `site != SITE`; strip `stepik_` prefix for `author_uid`; `comment_id` allowlist `^[A-Za-z0-9_-]{1,64}$` else drop; snippet server-side; enqueue non-blocking; return `200 {ok:true}` fast (<200ms, enqueue only). Queue-full → `push_queue_drop` + `200` (never `503`, avoids remark42 retry storm). Never log body/snippet/`user_name` on any failure path — log only `remote_hash8, bytes, reason`. |
| `GET /push/health` | shallow public `{ok:true}` rate-limited via `AllowPushByIP` only (no session). Deep `?deep=1` requires `auth.Guard` (`X-Gate-Auth`); Caddy strips inbound `X-Gate-Auth` (`request_header -X-Gate-Auth`) and sends NO `header_up X-Gate-Auth` (unlike `/healthz`), so via edge deep is unconditionally `404`; ops deep works via direct `http://gate:8081/push/health?deep=1` with token from docker/caddy-net (bypasses Caddy). | Deep returns `{ok:true, subs:<push_subs count via Bucket.Stats()>, queue:<len(chan)>}` — counts only, no subscriber PII. Deep without token → `404` (not `401`). Separate from `/healthz`. |
...
| `/static/*.html` | `text/html; charset=utf-8` | `public,max-age=3600` (via existing static handler) | `handleStatic` mapping fixed in §9. |
...
Gate-served `GET /sw.js` (`Content-Type: application/javascript`, `Cache-Control: no-store`), `REV` injected from `buildRevision()` via new `handlers.Server.Revision` field set from `main.revision` (fallback `"dev"` when VCS info absent; rollback changes `REV` → old cache deleted on activate). `STATIC_CACHE = "static-"+REV`.
...
- `STATIC_ASSETS` includes `/static/push.js` (omitting it would leave bell logic network-dependent offline). Every URL in `STATIC_ASSETS` must `200` with correct `Content-Type` — unit test asserts status + type. All files: `gate/static/style.css, push.js (new), icon-192.png, icon-512.png, icon-512-maskable.png, apple-touch-icon.png, favicon.svg` + Gate-served `/offline.html`. `badge: /static/icon-192.png` reuse accepted for MVP; verify legibility on Android in P1, add dedicated monochrome `badge-72.png` only if illegible.
...
- Send: for each job, for each sub: skip if `sub.uid == author_uid`; if `BestSession` present require `IsTeacher || cid ∈ AllowedClassIDs` else `push_skip_revoked`; if absent: push from stored array only if `max(created_at,last_ok_at) <24h` (`cid ∈ sub.cids`); stored `"all"` + absent → skip, except teacher (`uid == TEACHER_ID` from config) → push even when absent (teacher `all` exempt from 24h absent cap, but explicit logout/delete still silences). Teacher `"all"` covers future classes. Subscribe-time `cids ⊆ allowed` + send-time live check + resubscribe-time re-validate (defense in depth). Exactly one send per sub with its row key via `SendNotificationWithContext(ctx10s, payload, sub, {Subscriber: VAPID_SUBJECT, TTL: 86400, Urgency: normal, Topic: class-<cid>, VapidExpiration: default})`. Unknown `key_version` → `push_skip_unknown_key` + skip. Title truncated to ≤100 runes before marshal; `len(json)>2048` → cut snippet loop until fit. `last_ok_at` is updated only when `BestSession` was present at send time; absent-path success never touches `last_ok_at` (only `fail_count` on transient) — otherwise daily comments would self-refresh the 24h window and ex-students would never expire.
- Author: webhook `user_name` primary (plain-text sanitized as snippet), `BestSessionForUID(author_uid).FIO` fallback, else `""` (body = snippet alone). Title: `"<Class title> — новый комментарий"` single, `"<Class title>"` summary. Summary body: `1 новый комментарий / 2–4 новых комментария / 5+ новых комментариев` via `plural(n): if n%10==1 && n%100!=11 → "новый комментарий" else if n%10 in 2..4 && n%100 not in 12..14 → "новых комментария" else "новых комментариев"` (covers 11–14, 111); per-sub `N = count - ownCount`, same `tag:cid-<cid>` + `renotify:true`. Payload JSON ≤2KB. No avatar bytes, no tokens. Payload `url` asserted `strings.HasPrefix(url, "/class/")`.
...
File order (single pass, no revisits): `config` → `sessions` → `ratelimit` → `push` → `genvapid` → `handlers` → `admin` → `main` → `gate/Dockerfile` → `static/push.js` + `templates` + `static/offline.html` → `compose.yml` VAPID + `.env.example` (D1) → `Caddyfile` + `caddy-verify.sh` → `compose.yml` NOTIFY_* (D2) → `ci.yml` → `ops-runbook.md` → `TESTLOG.md`.
...
2. `gate/sessions/sessions.go`: create `push_subs`, `push_meta` buckets + `SchemaVersion` bump/create-if-missing (fresh → all buckets + `"2"`; `"1"` → create new buckets + `"2"`; `"2"` → no-op; else error; `Ping` asserts `"2"`); push CRUD + `SessionsForUID`/`BestSessionForUID` + atomic mutators `DeleteSessionAndPushSubs(sid)`, `StripClassAndPrunePush(uid,cid)`, `BumpVersionAndPrunePush(uid)` (single `Update` each; `push` exposes pure row-mutators only).
3. `gate/ratelimit/ratelimit.go`: `push` map + `AllowPush(sid) (bool, time.Duration)` (`Every(3s)`, burst 20) + `AllowPushByIP(ip) (bool, time.Duration)` (`Every(1s)`, burst 30, pre-auth); `ClientIP` = Caddy `CF-Connecting-IP` when from trusted peer (loopback/private) else `RemoteAddr` peer — remove `X-Forwarded-For` first-entry fallback (Caddy strips it; Gate ignores `XFF` entirely).
4. `gate/push/`: leaf types (§4.1 structs + `CidsOrAll.UnmarshalJSON`: `"all"` → All, array of int64 → list, missing/null/empty → error), `VerifyWebhook` (`r.Method==POST` else `404` + secret constant-time + empty-secret `404` + RemoteAddr-only private peer (`SplitHostPort` then `ParseIP`, deny parse-fail, loopback + private incl. docker `172.16/12`; never read `X-*`/`CF-*` on this route) + deny if any of `X-Forwarded-Uri/For/Proto/Host`, `Forwarded`, `Via`, `CF-Ray/CF-Connecting-IP/CF-Visitor/CF-IPCountry`, `X-Gate-Auth` present + require `Content-Type` via `mime.ParseMediaType==application/json` (allow `; charset=`), `404`, never-log-body), validation (`IsValidEndpoint` incl. allowlist + normalization + port `""`/`443` + resolve-then-IP-check / `IsValidKeys` / `IsValidCids`, `comment_id` allowlist `^[A-Za-z0-9_-]{1,64}$`), `TitleCache`.
5. `gate/cmd/genvapid/main.go`: generator (never commit output).
6. `gate/handlers/handlers.go`: `Server.Revision` field + `New` param (set from `main.revision`; precedence: ldflags-stamped `main.revision` → `debug.ReadBuildInfo vcs.revision[:12]+-dirty` → `"dev"` for `go run`); `Routes` `/push/*` + `/offline.html` (wrong-method `405`, missing/null `cids` → `400`, `Content-Type` via `mime.ParseMediaType==application/json`); manifest upgrade (§5, `application/manifest+json`); full `sw.js` (§6, incl. `startsWith("/class/")` guard + hash-preserving `notificationclick` focus/navigate); `handleOffline`; `handleStatic` MIME fix (`.js → application/javascript; charset=utf-8`, `.html → text/html; charset=utf-8`); JSON errors (§4 mapping + `X-Robots-Tag`); `push.js` wiring (+ `maxTouchPoints` iPad check, vapid-key `429` → silent skip/retry-next-load); `AllowPushByIP` before `LoadSession` on every `/push/*` + shallow health; deep health via `auth.Guard` (Caddy strips inbound token).
...
9. `gate/Dockerfile` REV stamp + `deploy/compose.yml` gate `VAPID_*` + `deploy/.env.example` keys + `deploy/Caddyfile` (§8 block, insertion after `/auth/*` before `/discuss` blocks, strip on `/push/health`, no `header_up`) + `deploy/caddy-verify.sh` (checks 6–9 + stub) + `docker compose config`; `caddy validate`; CF `BYPASS/DYNAMIC` verify.
...
1. Rotation runbook section in `docs/ops-runbook.md` (single-key MVP + future overlap, `_OLD` reserved; `PUSH_WEBHOOK_SECRET` simultaneous-restart window; hash-compare debug; `:8082` future option noted; direct Stepik-UI removal forbidden — only `/discuss/admin/revokeClass` revokes; Cloudflare Bypass + Bot Fight Mode OFF verification).
2. `push/health` deep evidence (direct `http://gate:8081/push/health?deep=1` with token from docker/caddy-net; edge deep unconditionally 404); TESTLOG waves; `caddy-verify.sh` re-run.
...
10. Burst: 5 comments in 30s → ≤2 pushes (1 immediate + 1 summary with frozen RU plural), visible 1 (same tag + renotify). Summary `N` excludes own comments per-sub.
...
- Authenticated SSRF via endpoint allowlist miss → closed by allowlist + resolve-then-IP-check (worker POSTs VAPID JWT to attacker host, floods/scans — blocked at subscribe `400` + send-time rebind check).
- Pre-auth flood on `/push/*` → closed by `AllowPushByIP` before session load.
- Direct Stepik-UI class removal (bypasses `admin.revokeClass`) → leaks up to 6h present / 24h absent; forbidden in ops, only admin revoke revokes. Teacher stolen-device `"all"` exempt lives until explicit `logoutAll`/delete.
...
| D7 | CSRF + rate | `subscribe\|unsubscribe` = IP + Origin + header CSRF + per-sid rate; `resubscribe` = IP + required-Origin + sid + per-sid rate only | `ClientIP` = Caddy `CF-Connecting-IP` when from trusted peer (loopback/private), else `RemoteAddr` peer (Gate ignores `X-Forwarded-For` entirely; Caddy strips it). `AllowPushByIP Every(1s)` burst 30 before `LoadSession` on all `/push/*` + shallow health; then `validSameOrigin` + `VerifyHeaderCSRF` (`__Host-csrf` via `document.cookie`) + per-sid `AllowPush Every(3s)` burst 20. `resubscribe` CSRF-exempt but requires present `Origin` exact match against `cfg.Origin` (no `Referer` fallback; SW cannot send header; `SameSite=Lax` + required Origin sufficient; if ever `None`, re-add CSRF). SW uses `credentials:'include'`. `vapid-key` also per-sid `AllowPush` after IP check. `429` + `Retry-After`. No `/auth/me` change. |
...
| D12 | Dependency | `webpush-go v1.4.0` interim + `govulncheck` in CI | MIT; pin `go.mod`+`go.sum`. `GenerateVAPIDKeys` = `RawURLEncoding`. `Options{Subscriber mailto:, TTL, Topic, Urgency}`. `VAPID_SUBJECT=mailto:mjgavrilov@gmail.com` per §7.1. No hand-rolled VAPID. `bluemonday.StrictPolicy` preferred for HTML strip if needed. |
...
| D16 | Send-time ACL | Present → require allowed else suppress; absent array + `max(created,last_ok)<24h` → push from stored; absent `"all"` → skip except teacher (exempt); logout deleted → nothing; revoke prunes atomically | `push_skip_revoked` on suppress. `revoke-class` deletes `"all"` rows + synchronously rewrites sessions `AllowedClassIDs` in same tx via `StripClassAndPrunePush` (never version-bump; healed on next visit). Sweep `fail>10 && ok>30d` + `no session && max(created,last_ok)>24h` (teacher `all` exempt). Ex-student >24h without re-auth gets nothing. `last_ok_at` only on present-session sends. |
...
| D18 | Resubscribe | `POST /push/resubscribe` UPDATE-by-`old_endpoint`; empty old → `400` + full endpoint re-validation | `old==""` → `400` (no INSERT; page-load full subscribe with `cids` + CSRF is the healer); foreign `old` → `403`; new `endpoint` re-runs full `IsValidEndpoint` + `IsValidKeys` (`400` on fail) then copied `cids` re-validated (`403`). SW best-effort (400-tolerant); page primary healer. |
| D19 | Titles | In-memory `cid → title` cache, `Класс <cid>` fallback | Set at subscribe + login + `EnsureFresh` success (only when non-empty and changed); send only reads; lost on restart; no Stepik hot-path. Truncate ≤100 runes, plain-text via `html/template`. |
...
| D25 | Webhook guard | New `VerifyWebhook` (`POST`-only + secret constant-time + empty-secret `404` + RemoteAddr-only private peer + extended header-deny + `mime.ParseMediaType` JSON, `404`) | `SplitHostPort` then `ParseIP`, reject parse fail, never read `X-*`/`CF-*`. Deny if `X-Forwarded-Uri/For/Proto/Host`, `Forwarded`, `Via`, `CF-Ray/CF-Connecting-IP/CF-Visitor/CF-IPCountry`, `X-Gate-Auth` present + `r.Method==POST` else `404` + `mime.ParseMediaType==application/json` + fail-closed `404` if secret empty. Never reuse `auth.Guard`; never via Caddy. |
| D26 | Coalesce UX | Immediate-first + 30s tail-batch, frozen RU summary + plural fn | Single → <5s; burst 5/30s → 2 sends (total N per-sub excl. own), visible 1. Same `tag:cid-<cid>` + `renotify:true`. Summary body: `1 новый комментарий / 2–4 новых комментария / 5+ новых комментариев` + fn `n%10==1&&n%100!=11→комментарий; n%10 in 2..4&&n%100 not in 12..14→комментария; else комментариев`. |
...
| D36 | Webhook single-port | Single `:8081` in MVP (no `:8082`); extended deny + `POST`-only + prefix CT | Extra listener = compose/healthcheck/complexity for <20 users; header-deny + secret + edge 404 sufficient. `:8082` is future option (runbook note only). `VerifyWebhook` denies if `X-Forwarded-Uri/For/Proto/Host`, `Forwarded`, `Via`, `CF-Ray/CF-Connecting-IP/CF-Visitor/CF-IPCountry`, `X-Gate-Auth` present + `r.Method==POST` else `404` + `mime.ParseMediaType==application/json` + fail-closed `404` if secret empty. |
...
| D40 | Absent cap | `max(created_at,last_ok_at)<24h` for stored-array push; sweep `max(...)>24h`; no self-refresh | Closes ex-student leak and fixes fresh-sub-never-fires (pure `last_ok` would suppress a fresh sub that never fired). `last_ok_at` only on present-session sends. Teacher `all` exempt. |
...
| D42 | Absent `last_ok` + resubscribe validation | No self-refresh; resubscribe full endpoint check | Absent-path success never touches `last_ok_at`. Resubscribe new `endpoint` → full `IsValidEndpoint`+`IsValidKeys` then `cids` re-check. Summary body per D26. Tests: `absent-no-refresh`, `ex-student-daily-suppressed`, `resubscribe-attacker-400`. |
| D43 | Teacher `all` exempt | Teacher stored `"all"` exempt from 24h absent cap | Explicit logout/delete still silences. Sweep skips `uid==TEACHER_ID` 24h rule. Student `"all"` + absent → skip. |
...
| D46 | Health strip | Strip inbound `X-Gate-Auth` on `/push/health*` | `request_header -X-Gate-Auth` + no `header_up` → edge deep unconditionally `404` (strongest; token-guess via edge impossible). Ops via direct `gate:8081`. Chosen over token-guarded because security first and ops cost is one `docker exec` line. |
...
Assumptions lists only env/edge/device assumptions; normative caps/keys/limits live in §12, not repeated here.

- Quiet-hours none in MVP — acceptable for school use; revisit on night-spam complaint.
- Teacher `"all"` auto-includes future classes (live `BestSession` expansion) + exempt from 24h absent cap; explicit logout/delete still silences — desired; no extra setting.
...
- `VAPID_SUBJECT=mailto:mjgavrilov@gmail.com` — no new contact needed.
- Cloudflare Bypass rule `(http.host eq "stepik.study67.fyi")` covers `/push/*`, `/sw.js`, `/manifest.json`, `/offline.html`; `BYPASS`/`DYNAMIC` recorded in TESTLOG P1/P2; Bot Fight Mode OFF — `POST /push/subscribe` not challenged, verified in P2.
- iPhone iOS 26.+ + Android Chrome test devices.
- Endpoint allowlist intentional — zero UX cost, closes authenticated SSRF.
- Single `:8081` for webhook intentional — `:8082` is future option only.
- Offline logout leaves rows until online logout/sweep/24h cap (documented gap, no code in MVP).
- D0 gates D1: no Gate changes without `ID` charset + `Locator.URL` shape + `Orig`/`Unix` + anchor screenshot verdict.
