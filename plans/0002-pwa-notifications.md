...
Date: 2026-09-21
Status: DRAFT for review — implements plan 0001 §7.4 hook (`manifest.json` + stub `sw.js` → full PWA + WebPush).
Decisions 2026-09-21 (teacher): notify scope = all members except author; no quiet hours (revisit later); teacher Telegram channel designed but deferred to plan 0003.
...
> Delegation note: `@requirements-clarifier` / `@architect-designer` / subagents unavailable in this environment (free-tier `task` tool rejected). Builder performed requirements + architecture directly from repo exploration + remark42/MDN docs. Review gates still apply: `go build/vet/test`, `caddy-verify.sh`, browser TESTLOG waves.
...
1. **PWA installable** on Android (Chrome/Edge/Samsung) + iOS (Safari 16.4+, incl. iOS 26 HS-default) + desktop (Chrome/Edge Add-to-Dock) — `standalone`, own icon, splash, `start_url:/`.
...
- Gate serves today: `GET /manifest.json` (`standalone, scope:/, start_url:/`, icons `/static/icon-192.png`, `/static/icon-512.png purpose:any maskable`), `GET /sw.js` stub (`fetch` passthrough), `apple-touch-icon.png`, `theme-color #3776AB`, `<link rel=manifest>` in `index.html`/`class.html`. Chromium install prompt **will not fire** with stub SW — needs real SW registration + `fetch` handler (MDN install criteria: HTTPS + manifest w/ 192+512 + `display` + registered SW).
...
- remark42 `v1.16.4` notification matrix (verified in docs, `remark42.com/docs/configuration/parameters` + `go-pkgz/notify`):
  ...
  - `NOTIFY_WEBHOOK_URL + NOTIFY_WEBHOOK_TEMPLATE + NOTIFY_WEBHOOK_HEADERS + NOTIFY_WEBHOOK_TIMEOUT (5s) + NOTIFY_QUEUE (100)` — admin webhook fires **HTTP POST JSON per new comment**. This is the event source for Gate fan-out. Template is Go `text/template`, e.g. `{"text": {{.Text | escapeJSONString}}}` — we override to include `site, page_url, comment_id, parent_id, user_id, time`.
...
- Scale/privacy: 2–3 classes, <20 users, RU, minors. Store only `uid+fio+avatar_url`. Threads never merge (`page_url` differs per `cid`). Cloudflare cache-bypass rule for `stepik.study67.fyi` must extend to new `/push/*` + `/sw.js` versioned URLs (all `BYPASS/DYNAMIC`, never cached).
- Deps frozen in 0001 §5.3: `bbolt, golang-jwt/v5, x/oauth2, x/time/rate` — **new dep `github.com/SherClockHolmes/webpush-go` needs explicit approval** (VAPID + RFC8291 encryption). Alternative: hand-rolled VAPID (rejected — crypto risk). No other deps.
...
- **Event source: remark42 admin webhook → Gate internal endpoint** (primary). Fallback: Gate poller `GET /api/v1/find?site=&url=` every 5m if webhook down (Phase 1B, cheap, reuses teacher JWT mint path internally — never exposes JWT to browser).
...
|-----|------|--------------------|
| `GET /manifest.json` | public, `Cache-Control: public,max-age=3600` | Full PWA manifest (see §5). `start_url:/?source=pwa`, `scope:/`, `display:standalone`, `orientation:portrait-primary`, `lang:ru`, `dir:ltr`, `theme_color/background_color`, icons 192+512 (+ `maskable` + `apple-touch-icon` 180), `id:/`, `categories:[education]`, `screenshots` deferred. |
| `GET /sw.js` | public, `Cache-Control: no-store` in MVP (versioned via `?v=<rev>` query at registration; long-cache + version pin later) | Real SW (see §6). Served by Gate (not static file) so `revision` can be injected. Scope `/`. |
...
| `POST /push/subscribe` | needs valid `sid`, Origin-checked (`https://stepik.study67.fyi`), CSRF (`X-CSRF-Token` from `/auth/me` cookie `__Host-csrf` or form token), rate-limited 20/min per session | Body JSON `{endpoint, keys:{p256dh, auth}, device:{ua, name?}, cids:[<cid…>] \| "all"}`. Validates endpoint URL `https://*` (FCM/WNS/APNs/Mozilla only — reject `http:`), key lengths (p256dh 87-88 chars b64url, auth 22-24), `cids ⊆ session.allowed` (teacher may use `"all"`). Upsert into `push_subs[sha256(endpoint)] → {uid, endpoint, p256dh, auth, cids|all, ua, created_at, last_ok_at}`. Returns `{ok:true}`. |
| `POST /push/unsubscribe` | same as subscribe | Body `{endpoint}`. Deletes by hash. Idempotent (200 even if missing). |
| `POST /push/webhook` | **internal only**: Docker-subnet guard + `X-Push-Webhook-Secret: <PUSH_WEBHOOK_SECRET 32B hex>` (from VPS `.env`, shared remark42 `NOTIFY_WEBHOOK_HEADERS` ↔ Gate env). **Never routed via Caddy** (no Caddy handle — direct `http://gate:8081` on `app` net). Fail-closed 404 on bad secret (same pattern as `X-Gate-Auth` Guard). | remark42 POST JSON `{site, page_url, comment:{id, parent_id, user_id, text_snippet, created_at}}` (template-defined, see §7). Gate: `ExtractCID(page_url)` → `cid`; drop if `cid` invalid/`site != SITE`; load comment author `stepik_<uid>`; fan-out async (queue, see §7). Returns `200 {ok:true}` fast (<200ms, enqueue only). |
| `GET /push/health` | Caddy-only (`X-Gate-Auth`) for `?deep=1` parity; shallow public `{ok:true}` | Returns `{ok:true, subs:<n>, queue:<n>}` for deep. Reuses `/healthz` pattern. |
...
Caddy delta: **no new `handle` for `/push/webhook`** (internal-only by design). Public `/push/subscribe`, `/push/vapid-key`, `/sw.js`, `/manifest.json` ride existing `handle @static`-style or `/auth/*`-style `reverse_proxy gate:8081` — add explicit `handle /push/* { reverse_proxy gate:8081 }` + keep CF `BYPASS`. Cloudflare Cache Rule `bypass-private` (hostname `stepik.study67.fyi`) already covers them; verify `cf-cache-status: BYPASS/DYNAMIC` on `/push/*` in TESTLOG.
...
- `handleManifest` extension (Go, `gate/handlers`):
  ...
  ```
  Icons already exist; add `maskable` generation step (safe-zone padding) + 180px `apple-touch-icon.png` already present. No new binary assets in repo beyond re-exported PNGs (keep `icon-192/512` + `apple-touch-icon` + `favicon.svg/ico`).
...
File: Gate-served `GET /sw.js` (Go handler, `Content-Type: application/javascript`, `Cache-Control: no-store`), version string injected from `buildRevision()`.

```js
const REV = "<git-rev>"; const STATIC_CACHE = "static-" + REV;
const STATIC_ASSETS = ["/static/style.css", "/static/icon-192.png", "/static/icon-512.png", "/static/apple-touch-icon.png", "/static/favicon.svg", "/offline.html"];
...
self.addEventListener("fetch", (e) => {
  ...
  if (u.pathname.startsWith("/static/") || u.pathname === "/offline.html" || u.pathname === "/favicon.ico") {
    e.respondWith(caches.firstMatchThenNetwork(e.request)); return;
  }
...
| Key | Note |
|-----|------|
| `VAPID_PUBLIC_KEY` / `VAPID_PRIVATE_KEY` | base64url P-256 pair, generated once via `webpush.GenerateVAPIDKeys()`, helper `go run ./gate/push -gen-vapid`. Public served at `/push/vapid-key`; private never leaves VPS. Rotation: dual-publish old+new public for 7d, then drop old (documented in runbook). |
| `VAPID_SUBJECT` | `mailto:mjgavrilov@gmail.com` (RFC8292 contact). |
| `PUSH_WEBHOOK_SECRET` | 32B hex (`openssl rand -hex 32`), shared: Gate env ↔ remark42 `NOTIFY_WEBHOOK_HEADERS=X-Push-Webhook-Secret:<value>`. |
| remark42 additions | `NOTIFY_ADMINS=webhook`, `NOTIFY_WEBHOOK_URL=http://gate:8081/push/webhook` (internal docker DNS!), `NOTIFY_WEBHOOK_TEMPLATE={"site":"{{.SiteID}}","page_url":"{{.PageURL}}","comment":{"id":"{{.ID}}","parent_id":"{{.ParentID}}","user_id":"{{.UserID}}","text_snippet":"{{.Text | truncate 300 | escapeJSONString}}","created_at":"{{.Timestamp}}"}}`, `NOTIFY_WEBHOOK_HEADERS=X-Push-Webhook-Secret:${PUSH_WEBHOOK_SECRET}`, `NOTIFY_WEBHOOK_TIMEOUT=5s`, `NOTIFY_QUEUE=200`. `TRUSTED_PROXY` unchanged. |
...
Unsubscribe removes endpoint row; disabling one class PATCHes `cids` (server filters `⊆ allowed`).

### 7.3 Fan-out worker (Gate, `gate/push/` new package)

- BoltDB buckets: `push_subs[endpoint_hash] → {uid, endpoint, p256dh, auth, cids|all, ua, created_at, last_ok_at, fail_count}`, `push_meta{vapid_public, vapid_private_ref}` (private key stays in env, never in DB), `push_queue` in-memory channel (cap 512) + worker pool (4 senders, shared outbound 4 req/s budget with Stepik client — push egress to FCM/APNs is separate host, no Stepik limit impact, but cap concurrency to 8 to fit 256M gate).
- Webhook handler: parse → `cid = ExtractCID(page_url)` → drop if unknown/`site` mismatch → enqueue `{cid, comment_id, author_stepik_id, snippet, page_url}`. Worker: for each job, list subs where (`cids=="all"` OR `cid ∈ cids`) AND `uid != author_uid` (don't notify self) AND `uid` still allowed for `cid` (check live `sessions` row: `cid ∈ AllowedClassIDs` + `EnsureFresh`-lite read without Stepik call — stale ≤48h accepted, definitive revoke already shrunk `allowed` so check is local) → `webpush.SendNotification(payload, sub, {VAPID keys, Subscriber, TTL: 86400, Urgency: normal, Topic: class-<cid>})`.
...
- Rate guard: per-`cid` coalesce window 30s (10 comments in 30s → 1 push with `body: "5 новых комментариев в <Class>"`). Prevents spam during lively threads.
...
- Webhook payload contains comment snippet — treated as private: Gate logs only `cid, comment_id, author_uid, sub_count`, never snippet text. Push payload snippet truncated 120 chars (matches what member would see on page anyway — no new disclosure, just new channel).
...
- 152-FZ/minors: no new PII collected (endpoint+p256dh+auth are device keys, not child data; stored per-uid, deletable via Unsubscribe + `deleteme` sweep on logout). Consent line extended (see §9).

## 8. Caddy + Cloudflare + compose delta (minimal)

- `deploy/Caddyfile`: add `handle /push/* { reverse_proxy gate:8081 }` (public subscribe/vapid-key; webhook NOT here). Keep `request_header CF-Connecting-IP {client_ip}` + rate-limit IP chain. No change to `/discuss/*` auth, strip rules, or healthcheck listener. After edit: `deploy/caddy-verify.sh` + container `caddy validate` (AGENTS.md exact forms).
- `deploy/compose.yml`: remark42 env += `NOTIFY_ADMINS=webhook`, `NOTIFY_WEBHOOK_URL=http://gate:8081/push/webhook`, `NOTIFY_WEBHOOK_HEADERS=X-Push-Webhook-Secret:${PUSH_WEBHOOK_SECRET}`, `NOTIFY_WEBHOOK_TIMEOUT=5s`, `NOTIFY_QUEUE=200`, `NOTIFY_WEBHOOK_TEMPLATE=<template>`; gate env += `VAPID_PUBLIC_KEY, VAPID_PRIVATE_KEY, VAPID_SUBJECT, PUSH_WEBHOOK_SECRET`. No new services, no `ports:` changes, resource limits unchanged (`gate 256M/128M` — push worker fits; measured separately in Wave P1 via `docker stats`).
- `deploy/.env.example`: keys only (`VAPID_*`, `PUSH_WEBHOOK_SECRET`, `NOTIFY_*` refs). Secrets never committed.
...
Login page consent append (one line): `Если включите уведомления, браузер получит технический ключ для доставки оповещений о новых комментариях в ваших классах. Отключить можно в любой момент на странице класса.`
...
- **D0 — spike (0.5d, standalone, before Gate changes):** minimal Go sender with `webpush-go` + real browser subscription (manual `vapid-key` dump) → prove VAPID send → notification shows on Android + iOS-HS. Verdict (one paragraph) appended here; split to `plans/0004-push-spike.md` only if >1 page (0003 reserved for teacher Telegram channel). Gates D1.
- **D1 — PWA shell (1d):** real `sw.js` + `/offline.html` + manifest upgrade + templates install UX + Caddy `/push/*` handle + CF verify. Acceptance: Wave P1 green. No webhook yet (subscribe stores rows, fan-out no-op with log `push_noop_no_event`).
- **D2 — push pipeline (2d):** `gate/push/` package (`subscribe/unsubscribe/vapid-key/webhook/worker`), BoltDB buckets, remark42 `NOTIFY_*` compose wiring, coalesce + prune + retry, RU strings, consent line. Acceptance: Wave P2 green (items 6–12).
- **D3 — hardening (0.5d):** VAPID rotation doc, `push/health` deep, runbook §, TESTLOG waves, `caddy-verify.sh` re-run, `docker stats` evidence. Merge.
...
- remark42 webhook template drift / `page_url` shape change → Gate `ExtractCID` fail-closed drops event + `warn push_webhook_bad_url` (no crash, no leak). Poller fallback (5m `find?site=&url=`) covers webhook outage (stale ≤5m accepted for <20 users).
...
- VAPID private leak → rotation procedure (§7.1) + dual-publish; old key dies after 7d (subs re-seal on next subscribe — transparent).
- SW cache poisoning private HTML → mitigated by design: SW fetch handler allowlists `/static/*` + `/offline.html` only; all else passthrough; `Cache-Control: no-store` on `/sw.js` itself; code review must assert no `caches.add('/')` or `/class` anywhere (add `grep -r "caches.*class" gate/` to CI).
...
- New dep `webpush-go` review: MIT, `SherClockHolmes/webpush-go v1.3.0`, ~21 imports, no cgo; pin via `go.mod` + `go.sum`, `go vet` + `govulncheck` in CI. Approval required per 0001 §5.3.

## 13. Decisions (resolved 2026-09-21, normative — was open questions)
...
3. Teacher Telegram channel: **design frozen in `plans/0003-teacher-telegram.md`, implementation deferred** (needs teacher BotFather bot + VPS `.env` token).
