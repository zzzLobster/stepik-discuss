...
// Count is the additive app-icon badge increment (SW adds it to the
// IndexedDB "push-badge" counter, then setAppBadge(total); absent counts as
// 1). Immediate → 1; summary per-sub → (U-O)-(first!=sub?1:0) floor 0 (D49).
...
// badgeBump: IndexedDB "push-badge" v1 kv/unread += inc, then setAppBadge(total).
// Best-effort only (never breaks showNotification, never throws on unsupported);
// no localStorage/document.cookie. Counter is per browser profile (last-write-wins).
...
- Page JS: new `gate/static/push.js` (cached `public,max-age=3600`, SW-cached, served `application/javascript; charset=utf-8`) holds `urlBase64ToUint8Array`, `csrfFromCookie` (page-only, reads `__Host-csrf` via `document.cookie`), bell logic, page-load migration, `clearBadgeAndCounter` on any authenticated page open (`clearAppBadge()` + close delivered `cid-*` notifications + shared IndexedDB counter → 0; logged-out touches nothing — D49), plus a `serviceWorker` `message` listener for `{type:"push-click"}` that `location.reload()`s when already on that class pathname (else `location.assign(url)`) so notification tap always shows a fresh thread (P2 fix: same-pathname hash navigation never reloads, and remark42 has no live thread refresh). `class.html`/`index.html` add `<script src="/static/push.js" defer>` + minimal inline `data-*` (`data-cid`, `data-uid`, `data-allowed` for all-minus-one) + bell button HTML. Class pages add user-controlled ↻ Обновить (`button[data-refresh]`, `.button` style) → `location.reload()` for standalone PWA with no browser chrome; no auto-reload on `visibilitychange` (preserves typed remark42 drafts, P2 fix). SW bundle never contains `localStorage`/`document.cookie` (enforced by unit test).
...
| D49 | App-icon badge | Truthful per-comment counts + simple clear behind `setAppBadge`/`clearAppBadge` guards; SW IndexedDB `push-badge` v1 `kv`/`unread`; iOS 16.4+ HS-installed (incl. WorkerNavigator, needs notification permission + per-app Badges toggle), desktop Chrome/Edge-installed; Android Chrome OS-dots automatically (nothing to code), Firefox no API | Immediate `count=1`; summary `count=(U-O)-(first!=sub?1:0)` floor 0, skip `N<=0`/`badge<=0`; any authenticated page open runs `clearAppBadge()` then counter → `0` (logged-out touches neither); per-profile last-write-wins accepted (2-user scope). Tests: `sw_test` badge block, `badge_test` formula + first-own + N==0 skip, payload marshal. |
