...
Refinement note (this pass): explore audit confirms all §2 current-state claims against the codebase; security audit HIGHs H1–H6 are incorporated below; structural defects are fixed by consolidation only. Prior freezes D34–D46, ClientIP, REV, SchemaVersion, caddy-verify 6–9, resubscribe, logout-silence (git log, 11 refinement commits 2026-09-22) are preserved, not reopened — duplicates are merged, not re-debated. D43 teacher-`all` exempt is the single intentional amendment (removed for security; see D40/D43).
...
| D3.4 | `.github/workflows/ci.yml` | Add `govulncheck` job (pinned version, e.g. `go install golang.org/x/vuln/cmd/govulncheck@v1.1.6 && govulncheck ./...` — never `@latest`). | CI green. |
...
All decisions below are final. Implementation must match; any change requires plan update. Merged rows preserve history: the listed primary row is authoritative; the merged number points to it.
...
| D12 | Dependency | `webpush-go v1.4.0` interim + `govulncheck` in CI | MIT; pin `go.mod`+`go.sum`. `GenerateVAPIDKeys` = `RawURLEncoding`. `Options{Subscriber mailto:, TTL, Topic, Urgency}`. `VAPID_SUBJECT=mailto:mjgavrilov@gmail.com` per §7.1. No hand-rolled VAPID. `bluemonday.StrictPolicy` for HTML strip. |
...
| D18 | Resubscribe | `POST /push/resubscribe` UPDATE-by-`old_endpoint`; empty old → `400` (not INSERT) + full endpoint re-validation (merged D37) | `old==""` → `400` (page-load full subscribe with `cids` + CSRF is the healer); foreign `old` → `403`; new `endpoint` re-runs full `IsValidEndpoint`+`IsValidKeys` then `cids` re-check (`403` on fail). SW best-effort (400-tolerant); page primary healer. Origin exact + `null`-reject per D7. |
...
| D36 | Merged into D25 | See D25 webhook-guard | Single-port `:8081`, extended deny + `POST`-only + `mime.ParseMediaType` CT. No `:8082` in MVP. |
| D37 | Merged into D18 | See D18 resubscribe | `old_endpoint==""` → `400` (not INSERT). |
...
| D39 | Health localhost-only | Strip + no inject; edge deep unconditionally 404 (merged D46) | Caddy `handle /push/health*` with `request_header -X-Gate-Auth` + NO `header_up X-Gate-Auth` (unlike `/healthz`). Shallow rate-limited via `AllowPushByIP`. Deep `?deep=1` via edge unconditionally 404 even with token; ops via direct `gate:8081`. Deep = subs count + queue len only, no PII. |
| D40 | Absent cap | Uniform `max(created_at,last_ok_at)<24h` for stored-array push; sweep `max(...)>24h`; no self-refresh; no teacher exempt (merged D42; supersedes D43) | Closes ex-student leak. `last_ok_at` only on present-session sends. Sweep uniform for all uids. Holiday semantics per §7.2. Tests: `absent-no-refresh`, `ex-student-daily-suppressed`. |
...
| D42 | Merged into D40 | See D40 absent-cap | Absent-path success never touches `last_ok_at`. Resubscribe new `endpoint` → full `IsValidEndpoint`+`IsValidKeys` then `cids` re-check. |
| D43 | SUPERSEDED by D40 | Teacher `"all"` exempt removed (intentional H3 amendment) | No absent-cap exempt for any uid. Explicit logout/delete still silences immediately. Preserved here only for history. |
...
| D45 | Merged into D25 | See D25 webhook-guard | `r.Method==POST` else `404`; deny `X-Forwarded-*/Forwarded/Via/CF-*/X-Gate-Auth`; `mime.ParseMediaType==application/json`. Tests: `Forwarded-404`, `charset-200`, `GET-404`. |
| D46 | Merged into D39 | See D39 health-strip | `request_header -X-Gate-Auth` + no `header_up` → edge deep unconditionally `404`. |
...
- `VAPID_SUBJECT=mailto:mjgavrilov@gmail.com` — no new contact needed (see D17).
